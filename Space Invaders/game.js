/**
 * @Date:   2020-08-20T17:29:53+01:00
 * @Last modified time: 2020-08-21T20:44:11+01:00
 */
let Game = function(){
  let _= this;
  this.scl = 0.1;
  ...
  this.init = function(){
    ...
    for (let x = 0; x < dd.cols; x++){
      for (let y = 0; y < dd.rows; y++){
        let invader = {x:x * dd.w+x * dd.gap, y:y * dd.h * dd.gap, w:dd.w, h:dd.h};
        invader.x += xoffset;
        invader.y += yoffset;
  ...
  this.draw = function(cc){
    for(let i =0; i <_.invaders.length; i++){
      let inv = _.invaders[i];
      drawImg(cc, _.invader_img, inv.x, inv.y, inv.w, inv.h);
      }
  ...
  this.updateInvaderBehavior = function(){
